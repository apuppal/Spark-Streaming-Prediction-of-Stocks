...
trainingData = ssc.textFileStream("/streamapp/train.txt").map(Vectors.parse)
testData = ssc.socketTextStream("localhost", 9997)
model = StreamingKMeans(k=2, decayFactor=1.0).setRandomCenters(3, 1.0, 0)
model.trainOn(trainingData)
print(model.predictOnValues(testData.map(lambda lp: (lp.label, lp.features))))
model.predictOnValues(testData.map(lambda lp: (lp.label, lp.features))).saveAsTextFiles("ts","ts2")
ssc.start()
